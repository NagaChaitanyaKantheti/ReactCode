...
class CountrySearch extends Component{
    ...
    render(){
        ...
        let startList=this.state.countriesList
        let query=this.state.query
        //let regex=new RegExp(query,'i')
        startList=startList.filter((entry)=> entry.name.toLowerCase().startsWith(query.toLowerCase()))

        let endList=this.state.countriesList
        endList=endList.filter((entry)=> entry.name.toLowerCase().endsWith(query.toLowerCase()))

        let containList=this.state.countriesList
        containList=containList.filter((entry)=> entry.name.toLowerCase().includes(query.toLowerCase()))
       
        
        return(
            <div>
                <input type="text" onChange={this.setdata} value={this.state.query}></input>                
                <ul>
                    <h5>Countries Starts with {query}</h5>
                ...
                <ul>
                    <h5>Countries ends with {this.state.query}</h5>
                    {endList.map((item,i)=> 
                        {return <li key={i}>{item.name} </li>}
                    )}
                </ul>
                 ...
                 <ul>
                    <h5>Countries contains {this.state.query}</h5>
                    {containList.map((item,i)=>
                        {return <li key={i}>{item.name} </li>}
                    )}
                </ul>
